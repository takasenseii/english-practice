  ...
  render(root) {
    ...
    function renderQuestion() {
      ...
      if (activity.type === "multiple-choice") {
        answerArea = `<div class="tutor-options">${activity.options.map((option, optionIndex) => `
          <label class="tutor-option">
            <input type="radio" name="tutorAnswer" value="${optionIndex}" ${Number(response.answer) === optionIndex ? "checked" : ""}>
            <span>${option}</span>
          </label>`).join("")}</div>`;
      } else {
        answerArea = `
      ...
      root.querySelector("#tutorQuestion").innerHTML = `
        ...
        <div class="tutor-actions">
          ...
          <button class="btn primary-btn" id="tutorCheck">${activity.type === "multiple-choice" ? "Check answer" : "Save and review"}</button>
        </div>
      ...
      root.querySelector("#tutorCheck").onclick = () => {
        ...
        if (activity.type === "multiple-choice") {
          ...
          responses[activity.id] = { answer, correct, completed: true, checkedAt: new Date().toISOString(), hintUsed: response.hintUsed || false };
